...
@UiController("spmu_ApplicationStageIncome.edit")
@UiDescriptor("application-stage-income-edit.xml")
@EditedEntityContainer("applicationStageIncomeDc")
@LoadDataBeforeShow
@DialogMode(forceDialog = true, resizable = true, closeOnClickOutside = true)
public class ApplicationStageIncomeEdit extends StandardEditor<ApplicationStageIncome> {
    @Inject
    ...
    @Subscribe
    protected void onInitEntity(InitEntityEvent<ApplicationStageIncome> event) {
    ...
    private void onAfterShow(AfterShowEvent event) {
        this.totalField.addValidator(value -> {
            Double sumQuarters = this.q1Field.getValue() + this.q2Field.getValue()
    ...
    protected void onBeforeShow(BeforeShowEvent event) {
        ...
        ApplicationStageIncome asi=this.getEditedEntity();
        if (!PersistenceHelper.isNew(asi)) {
            String title = "Статья ";
            title += ((asi.getOrder() != null ? " №" + asi.getOrder() : "") +" заявки на доходы" + (application.getCode() != null ? " №" + application.getCode() : ""));
            titleLabel.setValue(title);
            this.getWindow().setCaption("Статья " + (asi.getOrder() != null ? " №" + asi.getOrder() : "") +" заявки на доходы " + (application.getCode() != null ? " №" + application.getCode() : ""));
           }
        else{
            String title = "Создание статьи заявки на доходы";
            titleLabel.setValue(title);
        }


        this.applicationStageValues = this.appIncomeService.getParentList(this.getEditedEntity().getApplicationStageValue());
    ...
    private void onAfterCommitChanges(AfterCommitChangesEvent event) {
        this.appIncomeService.recalcStagesSum(this.getEditedEntity().getApplication());
    }
    ...
    private void sumAll(){
        this.totalField.setValue(this.q1Field.getValue() + this.q2Field.getValue()
                + this.q3Field.getValue() + this.q4Field.getValue() + this.year1.getValue()
                + this.year2.getValue() + this.year3.getValue() + this.year4.getValue()
                + this.totalForAllYearsField.getValue());
    }
